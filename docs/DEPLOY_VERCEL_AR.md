...
> ⚠️ **قيمة `NEXT_PUBLIC_SUPABASE_URL` هي رابط المشروع فقط** — مثال: `https://abcdxyz.supabase.co`.
> لا تلصق رابط REST من لوحة Supabase (`https://abcdxyz.supabase.co/rest/v1`) ولا تضع علامات تنصيص
> ولا مسافات أو أسطرًا جديدة. التطبيق يتسامح مع هذه الصور ويصحّحها تلقائيًا مع تحذير في السجلات
> (`[supabase] تم تصحيح NEXT_PUBLIC_SUPABASE_URL تلقائيًا…`)، لكن الأنظف أن تكون القيمة صحيحة من البداية.
...
يظهر في سجلات Vercel عند استعلامات الموقع العام (الباقات، العروض، الأعمال) لأن قيمة
`NEXT_PUBLIC_SUPABASE_URL` تحوي مسارًا، فيصبح الطلب `/rest/v1/rest/v1/<table>` ويرفضه PostgREST
برمز `PGRST125`. الحل:

1. **Project → Settings → Environment Variables**.
2. اجعل `NEXT_PUBLIC_SUPABASE_URL` = `https://abcdxyz.supabase.co` فقط:
   بلا `/rest/v1`، بلا شرطة في النهاية، بلا علامات تنصيص، بلا مسافات أو أسطر جديدة.
3. نفّذ **Redeploy** — قيم `NEXT_PUBLIC_` تُدمج **وقت البناء**، فلا يكفي تعديل المتغير وحده.

التطبيق منذ هذا الإصدار ينظّف القيمة تلقائيًا (`lib/supabase/env.ts`) ويطبع تحذيرًا واضحًا في السجلات،
ومع ذلك يُفضّل تصحيح المتغير في Vercel حتى لا تعتمد الصيانة على التصحيح التلقائي.
