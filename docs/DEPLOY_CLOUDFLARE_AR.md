...
2. من **SQL Editor** نفّذ `supabase/migrations/0001_init.sql` كاملًا (الجداول + RLS + bucket `store-assets`).
3. أنشئ حسابك من **Authentication → Users** ثم حوّله إلى مالك:
   ```sql
   update public.profiles set role = 'owner' where email = 'you@waathba.com';
   ```
4. (اختياري) نفّذ `supabase/migrations/0002_demo_data.sql` لإضافة المتاجر والعروض العربية الجاهزة.
